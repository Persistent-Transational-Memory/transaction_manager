...
#pragma once
#ifndef PTM_TRANSACTION_MANAGER_STD_TRANSACTION_MANAGER_H
#define PTM_TRANSACTION_MANAGER_STD_TRANSACTION_MANAGER_H
	...
	public:
		...
		static void running_task_error_handler(const TaskError &error) {
			if (error != TaskError::None && error != TaskError::Retry) {
				auto &logger = util::get_global_logger();
				logger.error("Error unexpected: ", static_cast<uint32_t>(error));
			}
		};
...
#endif //PTM_TRANSACTION_MANAGER_STD_TRANSACTION_MANAGER_H
