...
#pragma once
#ifndef PTM_TRANSACTION_MANAGER_PRELOAD_STD_TRANSACTION_MANAGER_H
#define PTM_TRANSACTION_MANAGER_PRELOAD_STD_TRANSACTION_MANAGER_H
	...
	private:
		...
		static void running_task_error_handler(const TaskError &error) {
			if (error != TaskError::None && error != TaskError::Retry) {
				util::logger_error("Error unexpected: ", static_cast<uint32_t>(error));
			}
		};
...
#endif //PTM_TRANSACTION_MANAGER_PRELOAD_STD_TRANSACTION_MANAGER_H
