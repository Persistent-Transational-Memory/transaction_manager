...
#pragma once
#ifndef PTM_THREAD_ALLOCATOR_STD_THREAD_ALLOCATOR_H
#define PTM_THREAD_ALLOCATOR_STD_THREAD_ALLOCATOR_H
		...
		inline void thread_bind(ThreadBindStrategy thread_bind_strategy, int id) {
			if (thread_bind_strategy == ThreadBindStrategy::NUMABind) {
				...
				if (!thread::THREAD_CONTEXT.bind_tid(id)) {
					util::logger::logger_error("Unable to bind tid ", id);
				}

				int cpu_id = thread::THREAD_CONTEXT.bind_cpu_on_node(numa_id);
				if (cpu_id == -1) {
					util::logger::logger_error("Unable to bind cpu on node ", numa_id);
				}
			}
				...
				if (cpu_id == -1) {
					util::logger::logger_error("Unable to bind cpu");
				}
			}
...
#endif //PTM_THREAD_ALLOCATOR_STD_THREAD_ALLOCATOR_H
